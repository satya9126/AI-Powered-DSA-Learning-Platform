...
# Load questions
with open("dsa_questions.json", encoding="utf-8") as f:
...
# Select Mode and API key
mode = st.selectbox("⚙ Select Mode", ["Offline Mode", "AI Assist"])
api_key = st.text_input("🔐 Enter your API Key", type="password") if mode == "AI Assist" else None

# Select topic and question
...
# Reset attempts if question changed
...
def local_code_checker(code, input_data, expected_output):
    try:
        ...
        if "=" not in input_data:
            call_code = f"result = {func_name}({input_data.strip()})"
        else:
...
# AI / Offline handler
def call_ai_api(mode, api_key, prompt, mode_name):
    if mode_name == "Offline Mode":
    ...
    elif mode_name == "AI Assist":
        if not api_key:
            return "❗ Please provide a valid API key for AI Assist"
...
if run_clicked and code.strip():
    ...
    if result and result.lower().strip() == "correct":
        ...
        st.error("❌ Your code might be wrong")
        if st.session_state.attempts >= 3:
            st.warning("📌 Showing correct solution after 3 failed attempts:")
            st.code(current_q["optimized_code"])
            st.markdown(f"📘 *Explanation (5 lines):*\n{current_q['optimized_explanation']}")

# Handle Hint button with strict 6 hints from AI
if hint_clicked:
    if mode == "AI Assist":
        prompt = (
            f"Give me exactly 6 helpful hints in simple English to solve this coding problem.\n"
            f"Keep each hint under 20 words. Number them 1 to 6.\n"
            f"Don't explain or solve the problem, just give helpful clues.\n\n"
            f"Problem: {current_q['question']}"
        )
        hints_response = call_ai_api(mode, api_key, prompt, mode)
        st.markdown("💡 *AI Hints:*")
        st.markdown(hints_response)
    else:
        st.info("💡 " + current_q["hint"])

# Handle Optimized button
if show_opt:
    st.code(current_q["optimized_code"])
    st.markdown(f"📘 *Explanation (5 lines):*\n{current_q['optimized_explanation']}")
